...
def save_chat_message(
    ...
    :param role: 消息角色，固定值：user（用户）/assistant（法律顾问）
    :param text: 对话核心内容，用户的提问或法律顾问的回答
    :param rewritten_query: 重写后的查询语句（可选，用于检索增强等场景，默认空字符串）
    :param item_names: 关联的法律法规名称列表（可选，支持多个，默认None）
    ...
    document = {
        ...
        "item_names": item_names,  # 关联法律法规名称列表
...
def update_message_item_names(ids: list[str], item_names: list[str]) -> int:
    ...
    :param item_names: 要设置的新法律法规名称列表
    ...
    try:
        ...
        result = mongo_tool.chat_message.update_many(
            ...
            {"$set": {"item_names": item_names}}  # 更新操作：设置新的法律法规名称列表
        )
        # 记录更新成功日志，包含更新数量和新的法律法规名称
...
if __name__ == "__main__":
    ...
    save_chat_message(sid, "assistant", "你好！我是海事法律顾问，很高兴为您服务。")
    # 3. 写入带关联法律法规的用户消息（手动指定ts=1002，测试item_names字段）
    save_chat_message(sid, "user", "海商法中承运人的责任是什么？", item_names=["中华人民共和国海商法"])
