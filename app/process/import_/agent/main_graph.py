...
# 1. 定义图的构建对象，并且指定全局state状态
import_graph_builder = StateGraph(ImportGraphState)

# 2. 添加节点
import_graph_builder.add_node("node_entry", node_entry)
import_graph_builder.add_node("node_pdf_to_md", node_pdf_to_md)
import_graph_builder.add_node("node_md_img", node_md_img)
import_graph_builder.add_node("node_document_split", node_document_split)
import_graph_builder.add_node("node_item_name_recognition", node_item_name_recognition)
import_graph_builder.add_node("node_bge_embedding", node_bge_embedding)
import_graph_builder.add_node("node_import_milvus", node_import_milvus)
...
def node_entry_after(state: ImportGraphState):
    ...
    if state.get("is_md_read_enabled", False):
        logger.info(
            f"传入的文件地址： {state.get('local_file_path')}，传入的文件是Markdown文件，所以跳转到node_md_img节点")
        return "node_md_img"
    elif state.get("is_pdf_read_enabled", False):
        logger.info(f"传入的文件地址： {state.get('local_file_path')}，传入的文件是PDF文件，所以跳转到node_pdf_to_md节点")
        return "node_pdf_to_md"
    else:
        logger.error(f"传入的文件地址： {state.get('local_file_path')}，无法识别文件类型")
        return END
...
if __name__ == "__main__":
    ...
    test_pdf_name = os.path.join("doc", "中华人民共和国海商法_20251028.pdf")
